...
하위 폴더까지 포함하여 한 번에 비교 및 복사가 가능하도록 기능을 개선하는 것을 목표로 한다. 
하위 폴더를 하나의 파일처럼 처리하여 목록에 표시하고, 파일과 폴더를 구분할 수 있도록 적절한 색상으로 표시한다. 
또한 복사 버튼을 누르면 선택한 하위 폴더 내부의 모든 내용(파일 및 하위 폴더 포함)이 함께 복사되도록 구현한다.